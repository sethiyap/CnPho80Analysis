...
#' @param header_count_file logical, whether the read count file contains header row or not.
#' If TRUE, the first line will be skipped during processing.
...
#' }
#'
deseq_from_htseqcount <- function(dir, pattern, metadata_file,header_count_file=FALSE,HTSeqOutput=TRUE,drop_genes=NULL, minimum_read_cutoff=2,write_output=FALSE, outfile) {
  ...
  df_count_matrix <- tibble::tibble(file_name = dd) %>%
    dplyr::mutate(file_cont = purrr::map(file_name,data.table::fread,data.table = F, skip=skip_row))  %>%
    tidyr::unnest(cols = c(file_cont), names_repair="universal") %>%
    dplyr::mutate(V1=forcats::as_factor(V1), file_name =gsub(pattern=pattern,replacement="", basename(file_name)))  %>%
  ...
  if(HTSeqOutput==TRUE){
    df_count_matrix <- df_count_matrix %>%
      dplyr::slice(1:(nrow(.)-5))#remove first fivelines of statistics
  } else{
    df_count_matrix=df_count_matrix
  }
